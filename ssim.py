...
#Copyright: https://github.com/Po-Hsun-Su/pytorch-ssim
...
for idx in range(1,pair_num+1):
    ...
    #print(img_generated)
    v = ssim(img_generated,img_paired)
    ssim_lost_list.append(v.item())    

if not os.path.exists('./ssim_list'):
    os.mkdir('./ssim_list')


print(f"{key}\tlen:{len(ssim_lost_list)}\tmin:{min(ssim_lost_list)}\tmax:{max(ssim_lost_list)}\tmean:{sum(ssim_lost_list)/len(ssim_lost_list)}")

np.save(f'ssim_list/ssim_{key}_{pair_num}.npy',np.array(ssim_lost_list))
